...
class Weather:
    def __init__(self, **kwargs):
        ...
        station_data = json.loads(
            self.api.get("/points/{}".format(self.coordinate)).text)
    ...
    def _validate_cache(self):
        ...
        if self.cacheTime - curtime < 0:
            self.cache = self.api.get("/gridpoints/{}/{},{}/forecast".format(
                self.gridId, self.gridX, self.gridY)).json()
    ...
    def is_raining():
        hourly = self.api.get("/gridpoints/{}/{},{}".format(
            self.gridId, self.gridX, self.gridY)).json()
        return "rain_showers" == hourly['properties']['weather']['values'][0][
            'value'][0]['weather']
